...
void BotMatch_HelpAccompany(bot_state_t *bs, bot_match_t *match) {
	...
	if (!GAMETYPE_IS_A_TEAM_GAME(gametype)) return;
	...
	if (match->type == MSG_HELP) {
		BotChat_SetAction(bs, client, LTG_TEAMHELP, TEAM_HELP_TIME);
	}
	else {
...
void BotMatch_GetItem(bot_state_t *bs, bot_match_t *match) {
	...
	trap_BotMatchVariable(match, NETNAME, netname, sizeof(netname));
	client = ClientOnSameTeamFromName(bs, netname);
	BotChat_SetAction(bs, client, LTG_GETITEM, TEAM_GETITEM_TIME);
}
...
void BotMatch_Patrol(bot_state_t *bs, bot_match_t *match) {
	...
	//get the team goal time
	bs->teamgoal_time = BotGetTime(match);
	BotChat_SetAction(bs, client, LTG_PATROL, TEAM_PATROL_TIME);
}
...
void BotMatch_GetFlag(bot_state_t *bs, bot_match_t *match) {
	...
	//
	client = FindClientByName(netname);
	// get an alternate route in  CTF, 1FCTF and eCTF
	if (GAMETYPE_USES_RED_AND_BLUE_FLAG(gametype)) {
...
void BotMatch_AttackEnemyBase(bot_state_t *bs, bot_match_t *match) {
	...
	else if (GAMETYPE_USES_OBELISKS(gametype)) {
...
void BotMatch_RushBase(bot_state_t *bs, bot_match_t *match) {
	...
	if(!GAMETYPE_USES_RED_AND_BLUE_FLAG(gametype) && !GAMETYPE_USES_OBELISKS(gametype)) {
		return;
	}
	...
	BotChat_SetAction(bs, client, LTG_RUSHBASE, CTF_RUSHBASE_TIME);
}
...
void BotChat_SetAction(bot_state_t *bs, int client, int ltg, int teamgoal) {
	...
	//set the team goal time
	if (!bs->teamgoal_time) {
		bs->teamgoal_time = FloatTime() + teamgoal;
	}
	//
	BotSetTeamStatus(bs);
