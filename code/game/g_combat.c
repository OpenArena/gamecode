...
void AddScore( gentity_t *ent, vec3_t origin, int score )
{
	...
	// show score plum
	if( level.numNonSpectatorClients<3 && score < 0 && !GAMETYPE_IS_A_TEAM_GAME(g_gametype.integer)) {
...
//unlagged - backward reconciliation #2
	...
	if( !((GAMETYPE_IS_ROUND_BASED(g_gametype.integer) && GAMETYPE_IS_A_TEAM_GAME(g_gametype.integer)) && level.time < level.roundStartTime) ) {
	...
	if (attacker && attacker->client) {
		...
		if ( attacker == self || OnSameTeam (self, attacker ) ) {
			if(g_gametype.integer != GT_POSSESSION && !GAMETYPE_IS_ROUND_BASED(g_gametype.integer) && level.time < level.roundStartTime) {
				if( (GAMETYPE_IS_A_TEAM_GAME(g_gametype.integer) && self->client->ps.persistant[PERS_SCORE]>0) || level.numNonSpectatorClients<3) {
               ...
               vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
	...
	if ( !(dflags & DAMAGE_NO_PROTECTION) ) {
		...
		if ( mod != MOD_JUICED && mod != MOD_CRUSH && targ != attacker && !(dflags & DAMAGE_NO_TEAM_PROTECTION) && OnSameTeam (targ, attacker)  ) {
			if ( ( !g_friendlyFire.integer && !(GAMETYPE_IS_ROUND_BASED(g_gametype.integer) && GAMETYPE_IS_A_TEAM_GAME(g_gametype.integer)) ) || ( g_elimination_selfdamage.integer<2 && GAMETYPE_IS_ROUND_BASED(g_gametype.integer) ) ) {
				return;
			}
		}
...
//So people can be telefragged!
	if (GAMETYPE_IS_ROUND_BASED(g_gametype.integer) && level.time < level.roundStartTime && ((mod == MOD_LAVA) || (mod == MOD_SLIME)) ) {
		damage = 1000;
	}
	...
	if(GAMETYPE_USES_RED_AND_BLUE_FLAG(g_gametype.integer)) {
