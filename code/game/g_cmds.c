...
void Cmd_Follow_f( gentity_t *ent ) {
	...
	if ( GAMETYPE_IS_ROUND_BASED(g_gametype.integer) && GAMETYPE_IS_A_TEAM_GAME(g_gametype.integer) && g_elimination_lockspectator.integer
...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText ) {
	...
	if ( !GAMETYPE_IS_A_TEAM_GAME(g_gametype.integer) && mode == SAY_TEAM ) {
		mode = SAY_ALL;
	}
	...
	case SAY_TEAM:
		...
		break;
	case SAY_TELL:
		if (target && target->inuse && target->client && GAMETYPE_IS_A_TEAM_GAME(g_gametype.integer) &&
...
void Cmd_CallVote_f( gentity_t *ent ) {
	...
	if ( Q_strequal( arg1, "g_gametype" ) ) {
		...
		if(!GAMETYPE_IS_A_TEAM_GAME(g_gametype.integer)) { //Not a team game
