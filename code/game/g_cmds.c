...
void SetTeam( gentity_t *ent, char *s ) {
	...
	else if (G_IsATeamGame(g_gametype.integer,qfalse)) {
...
void G_Say( gentity_t *ent, gentity_t *target, int mode, const char *chatText ) {
	...
	if ( !G_IsATeamGame(g_gametype.integer,qfalse) && mode == SAY_TEAM ) {
		mode = SAY_ALL;
	}
	...
	case SAY_TEAM:
		...
		break;
	case SAY_TELL:
		if (target && target->inuse && target->client && G_IsATeamGame(g_gametype.integer,qfalse) &&
