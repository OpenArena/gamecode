...
//unlagged - client options
	...
	if (G_IsATeamGame(g_gametype.integer,qfalse) && g_entities[clientNum].r.svFlags & SVF_BOT) {
	...
	// colors
	if(G_IsATeamGame(g_gametype.integer,qfalse) && g_instantgib.integer) {
		switch(team) {
			case TEAM_RED:
				c1[0] = COLOR_BLUE;
				c2[0] = COLOR_BLUE;
...
//   this means this client is not running in our current process
	...
	if ( G_IsATeamGame(g_gametype.integer,qfalse) && client->sess.sessionTeam != TEAM_SPECTATOR ) {
...
//	char	*savedAreaBits;
	...
	} else if (G_IsATeamGame(g_gametype.integer,qtrue)) {
