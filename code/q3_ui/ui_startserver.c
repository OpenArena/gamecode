...
static qboolean BotAlreadySelected( const char *checkName ) {
	...
	for( n = 1; n < PLAYER_SLOTS; n++ ) {
		...
		if( UI_IsATeamGame(s_serveroptions.gametype,qfalse) &&
...
//	trap_Cvar_SetValue( "dedicated", Com_Clamp( 0, 2, dedicated ) );
	...
	// set player's team
	if( /*dedicated == 0 &&*/ UI_IsATeamGame(s_serveroptions.gametype,qfalse) ) {
...
static void ServerOptions_InitPlayerItems( void ) {
	...
	if(s_serveroptions.multiplayer && !UI_IsATeamGame(s_serveroptions.gametype,qfalse)) {
...
static void ServerOptions_MenuInit( qboolean multiplayer ) {
	...
	y = 268;
	if(!UI_IsATeamGame(s_serveroptions.gametype,qtrue)) {
	...
	if( UI_IsATeamGame(s_serveroptions.gametype,qfalse) && s_serveroptions.gametype != GT_ELIMINATION && s_serveroptions.gametype != GT_CTF_ELIMINATION) {
		y += BIGCHAR_HEIGHT+2;
