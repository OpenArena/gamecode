...
static float CG_DrawScores(float y) {
	...
	if (CG_IsATeamGametype(cgs.gametype)) {
		...
		if (CG_GametypeUsesFragLimit(cgs.gametype)) {
			v = cgs.fraglimit;
		} else if (CG_GametypeUsesScoreLimit(cgs.gametype)) {
			v = cgs.scorelimit;
		} else {
			v = cgs.capturelimit;
		}
		...
		if (cgs.fraglimit && CG_GametypeUsesFragLimit(cgs.gametype)) {
		...
		if (cgs.scorelimit && CG_GametypeUsesScoreLimit(cgs.gametype)) {
		...
		if (cgs.capturelimit && CG_GametypeUsesCaptureLimit(cgs.gametype)) {
