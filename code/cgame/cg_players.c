...
static void CG_LoadClientInfo(int clientNum, clientInfo_t *ci) {
	...
	// sounds
	dir = ci->modelName;
	fallback = (CG_IsATeamGame(cgs.gametype,qfalse)) ? DEFAULT_TEAM_MODEL : DEFAULT_MODEL;
...
static qboolean CG_ScanForExistingClientInfo(clientInfo_t *ci) {
	...
	for (i = 0; i < cgs.maxclients; i++) {
		...
		if (Q_strequal(ci->modelName, match->modelName)
				...
				&& (!CG_IsATeamGame(cgs.gametype,qfalse) || ci->team == match->team)) {
...
static void CG_PlayerSprites(centity_t *cent) {
	...
	if (!(cent->currentState.eFlags & EF_DEAD) &&
			...
			CG_IsATeamGame(cgs.gametype,qfalse)) {
